...
tax_df$path <- paste(tax_df$Kingdom, tax_df$Phylum, tax_df$Class, tax_df$Order, tax_df$Family, tax_df$Genus, sep = "/") 
...
k <- unique(paste(tax_df$Kingdom))
kp <- unique(paste(tax_df$Kingdom, tax_df$Phylum,  sep = "/"))
kpc <- unique(paste(tax_df$Kingdom, tax_df$Phylum, tax_df$Class,  sep = "/"))
kpco  <- unique(paste(tax_df$Kingdom, tax_df$Phylum, tax_df$Class, tax_df$Order,  sep = "/"))
kpcof  <- unique(paste(tax_df$Kingdom, tax_df$Phylum, tax_df$Class, tax_df$Order, tax_df$Family, sep = "/"))
kpcofg   <- unique(paste(tax_df$Kingdom, tax_df$Phylum, tax_df$Class, tax_df$Order, tax_df$Family, tax_df$Genus, sep = "/"))

#combines hierarchical info
path_header <- c(k, kp, kpc, kpco, kpcof, kpcofg)
